...
class Node {
    ...
    public:
        ...
        int pdb555(){
            abstract_state(abs1, &this->state, &estado_abs1);
            abstract_state(abs2, &this->state, &estado_abs2);
            abstract_state(abs3, &this->state, &estado_abs3);

            return (*state_map_get(mapa1, &estado_abs1) + *state_map_get(mapa2, &estado_abs2) + *state_map_get(mapa3, &estado_abs3));
        };
        /*int manhattan(){
            float h = 0;
            for(int i = 0; i < 16; i++){
...
int main(int argc, char const *argv[]) {
    ...
    std::pair<float,int> respuesta;
    Node raiz;
    FILE *f1 = fopen("abstraccion_15Puzzle_1.pdb","r");
    FILE *f2 = fopen("abstraccion_15Puzzle_2.pdb","r");
    FILE *f3 = fopen("abstraccion_15Puzzle_3.pdb","r");
    abs1 = read_abstraction_from_file("abstraccion_15Puzzle_1.abst");
    mapa1 = read_state_map(f1);
    abs2 = read_abstraction_from_file("abstraccion_15Puzzle_2.abst");
    mapa2 = read_state_map(f2);
    abs3 = read_abstraction_from_file("abstraccion_15Puzzle_3.abst");
    mapa3 = read_state_map(f3);
    fclose(f1);
    fclose(f2);
    fclose(f3);
